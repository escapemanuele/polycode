...
│   ├── repo_config.rs which checkout `.polycode.toml` is read from: worktree, then source repository
│   ├── claude/      native discovery, argv, prompts, JSONL decoder, adapter
│   ├── codex/       native discovery, exec argv, prompts, JSONL decoder, adapter
│   └── verify/      deterministic command runner: `.polycode.toml` reader, argv runner with timeout, Markdown artifact
├── store/
