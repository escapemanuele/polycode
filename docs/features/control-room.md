...
Mission: `↑`/`↓` select a package, `Enter` open its current run, `S` start the selected ready package (uses the composer's Execution and Effort; the starting card follows it, and the package row says `working` once it runs), `u` answer what the selected package's run asks (the attention overlay; Enter approves, Ctrl-S skips, a typed response continues without granting), `I` bring the selected finished package in (Enter confirms; applies the run when needed), `A` auto-approve permission requests for this mission's runs (session-scoped intention: arms the runs it has now and the ones this session starts for it), `W` enter the Senate (on the missions list or a mission: opens the selected campaign's 3D view with `polycode world --mission <id>`; see world.md). Run actions (`s`, `a`, `X`, …) are not bound on the mission screens.
Run detail: `Enter`/`o` open selected stage artifact, `r` resume/recover, `s` stop, `t` retry selected failed stage (chooser: Configured provider / Claude / Codex, Enter retries; a stage whose model Codex refused also gets `Codex on <fallback model>`, highlighted on open), `u` attention overlay, `A` arm/disarm automatic approval of this run's permission requests, `l` raw logs, `d` workspace diff, `a` apply (Enter confirms), `b` rebase the run's change onto the checkout's current HEAD (Enter confirms; a conflict changes nothing, and verification has to run again before apply), `P` pull request, labelled with the run's publish state — `Push to PR` (not pushed), `Update #n · behind` after a later fix/continue/rebase, or `Pushed ✓ #n`, where `P` opens the PR instead of pushing again (Enter confirms a push; a spinning card times the publish, Esc hides it without stopping it; the result card holds the URL until dismissed: `o` opens it in the browser, `y` copies it, Enter/Esc close and leave the summary in the footer), `X` discard (Enter confirms), `f` next cycle (a picker of Fix / Continue / Follow-ups, the last only when the decision wrote that section; skipped when only one applies; books a fix on a run still working), `c`/`w` jump straight to continue/follow-ups, `e` expand the task in the rail, `i` technical details. The completed-run panel row shows only `a P f`; the footer below it holds only `X` Discard and `i` Details. `d`, `b`, `c` and `w` still work and are in help; `d` also works inside the apply and push confirmations. An apply refused because the checkout moved past the run's base opens the rebase confirmation with the reason (`WorkerSuccess::ApplyNeedsRebase`, from `PatchCheckFailed { rebasable: true }`); after the rebase, verification has to run again (`f`) before apply.
