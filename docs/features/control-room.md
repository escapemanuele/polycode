...
- The RESULT section distinguishes "the stage produced nothing" from "the panel cannot read the store". An artifact listing that fails (`state.artifacts_unavailable`) renders `Results unavailable — <error>` in danger colour and raises the error toast; it never falls through to `No verified artifact`. Run `01M34EB4YY6G7J3HE2XT2G5KFP` showed every completed stage as artifact-less because a development build had migrated `~/.polycode/polycode.db` to a newer schema and the released TUI, which reopens the store on every read, was silently failing `list_artifacts`. A dev worktree must not point its binary at the shared database.
